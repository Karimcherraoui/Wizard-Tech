const categories = [
    ...
    export const getCategories = ()=>{return categories.map(el=> { return {name:el.name,id:el.id}})}
    export const getCategory = (catId)=>{return categories.filter((el) => el.id === catId)}
    // export const getSession = ({catId,sessionId})=>categories.find((el)=> ( el.id === catId ))?.sessions.find((e)=> e.id === sessionId ) ?? "not Found"

export const getSession = ({ catId, sessionId }) => {
    const category = categories.find((el) => el.id === catId);

    if (category) {
        const session = category.sessions.find((e) => e.id === sessionId);
        if (session) {
            return session;
        }
    }
